...
#pragma mark - UIView 为普通的视图截图

- (void)snapshotForNormalView:(UIView *)normalView
{
    CGRect currentFrame = normalView.frame;
    
    
    UIGraphicsBeginImageContextWithOptions(currentFrame.size, YES, UIScreen.mainScreen.scale);
    [normalView.layer renderInContext:UIGraphicsGetCurrentContext()];
    UIImage *snapshotImage = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    
    normalView.frame = currentFrame;
