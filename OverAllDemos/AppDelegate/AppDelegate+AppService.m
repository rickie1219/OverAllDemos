...
- (void)loginStateChange:(NSNotification *)notification
{
    ...
    // 使YYFPS Lable只创建一次，然后每次跳转的时候，都让YYFPS移动window子视图的到最上方
    [kAppWindow bringSubviewToFront:kAppWindow.subviews.firstObject];
}
...
{
    NSString *strUrl = [NSString stringWithContentsOfURL:[NSURL URLWithString:@""] encoding:NSUTF8StringEncoding error:nil];
    
    if (nil != strUrl && [strUrl length] > 0 && [strUrl rangeOfString:@"version"].length == 7)
    {
        [self checkAppAndUpdateWithURLStr:strUrl];
    }
}

- (void)checkAppAndUpdateWithURLStr:(NSString *)appInfo
{
    // 获取当前版本
    NSString *strVersion = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleVersion"];
    NSString *strAppInfo = [appInfo substringFromIndex:[appInfo rangeOfString:@"\"version\":"].location + 10];
