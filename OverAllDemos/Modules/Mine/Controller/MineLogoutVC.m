...
@interface MineLogoutVC ()<UITableViewDelegate,UITableViewDataSource>
...
- (void)setupUI
{
    ...
    [self.tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:@"BaseTableViewCell"];
...
//    aboutCofigModel.isShowArrow = YES;
    ...
    NSArray *one = @[@{@"title":@"推送消息设置"}];
    NSArray *two = @[@{@"title":@"声音"},@{@"title":@"清除缓存"},@{@"title":@"关于我们"}];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [_dataArray[indexPath.section][indexPath.row] objectForKey:@"title"];
    return cell;
}
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //            break;
    //    }
    NSLog(@"点击了 ---> %@ ", [_dataArray[indexPath.section][indexPath.row] objectForKey:@"title"]);
}
